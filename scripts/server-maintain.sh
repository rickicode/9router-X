#!/bin/bash
# 9router-X disk auto-maintenance — hourly cron on prod (192.168.90.101).
# Installed at /usr/local/bin/9router-maintain.sh, cron: 15 * * * *.
...
set -u
LOG=/root/9router-x-maintain.log
exec >>"$LOG" 2>&1
...
rm -rf /root/9router-x/benchmarks /root/9router-x/db.sqlite3 2>/dev/null
