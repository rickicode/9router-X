    ...
    <div ref={containerRef} className={cn("relative flex flex-col gap-1.5", className)}>
      ...
      <div className="relative flex items-center">
        ...
        <div className="absolute inset-y-0 right-0 flex items-center pr-2 gap-0.5">
          {loading && (
            <span className="material-symbols-outlined text-[16px] text-text-muted animate-spin mr-1">
              progress_activity
            </span>
          )}
          ...
          {clearable && Boolean(value) && !disabled && (
            ...
            >
              <span className="material-symbols-outlined text-[15px] leading-none block">
                close
              </span>
            </button>
          )}
          ...
          >
            <span
              className={cn(
                "material-symbols-outlined text-[18px] leading-none block transition-transform duration-150",
                isOpen && "rotate-180",
              )}
            >
              expand_more
      ...
      {isOpen && (
        <ul
          ...
          className={cn(
            ...
            "rounded-[10px] border border-border bg-surface shadow-xl p-1",
            ...
            filteredOptions.map((opt, index) => {
              ...
              return (
                <li
                  ...
                  className={cn(
                    "px-3 py-2 text-xs rounded-lg flex items-center justify-between gap-2 cursor-pointer transition-colors select-none",
                    isActive
                      ? "bg-surface-2 text-text-main"
                      : "text-text-main hover:bg-surface-2",
                    isSelected && "text-brand-500 font-semibold bg-brand-500/10",
                  )}
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    {opt.icon ? (
                      ...
                      <span className="material-symbols-outlined text-[16px] text-brand-500 shrink-0">
                        edit_note
                      </span>
                    ) : null}
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 ml-2">
                    {opt.badge && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-surface-3 text-text-muted font-mono">
                        {opt.badge}
                      </span>
                    )}
                    {isSelected && (
                      <span className="material-symbols-outlined text-brand-500 text-[16px] leading-none">
                        check
                      </span>
                    )}
                  </div>
