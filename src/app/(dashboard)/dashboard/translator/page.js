...
export default function TranslatorPage() {
 ...
 if (!provider || !model) {
 notify.warning("Missing provider or model. Please run step 1 first to detect them.");
 return;
 }
 ...
 <div>
 <h2 className="text-sm font-semibold text-text-main">Replay</h2>
 ...
 <span className="material-symbols-outlined text-[18px] text-text-muted group-hover:text-primary">
...
function MetaBadge({ label, value, color }) {
 ...
 return (
 <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-sm text-xs font-mono ${colors[color]}`}>
 <span className="text-text-muted/70 font-sans text-[11px]">{label}:</span>{value}
 </span>
 );
}
