...
export default function BasicChatPageClient() {
 ...
 <div className="flex shrink-0 items-center justify-between gap-3 h-8 px-3 lg:px-3">
 <div ref={modelMenuRef} className="relative">
 <button
 type="button"
 onClick={() => setModelMenuOpen((value) => !value)}
 className="flex items-center gap-3 rounded-sm border border-border bg-surface h-8 px-3 text-left transition hover:bg-surface-2"
 >
 ...
 {modelMenuOpen ? (
 <div className="absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-sm border border-border bg-surface">
 <div className="border-b border-border h-8 px-3">
 <p className="text-xs text-text-subtle">Models</p>
 ...
 <div key={group.providerId} className="mb-2 rounded-sm border border-border bg-surface-3 p-3">
 ...
 className={`rounded-sm border px-3 py-3 text-left transition ${isActive ? "border-primary/30 bg-primary/10" : "border-border bg-surface hover:bg-surface-2"}`}
 >
 ...
 className="rounded-sm border border-border bg-surface h-8 px-3 text-sm text-text-main transition hover:bg-surface-2"
 >
 History
 </button>
 ...
 {historyOpen ? (
 <div ref={historyMenuRef} className="absolute right-4 top-[72px] z-20 w-[min(360px,calc(100vw-2rem))] rounded-sm border border-border bg-surface p-3 lg:right-6">
 <div className="px-3 h-8">
 <p className="text-xs text-text-subtle">Recent chats</p>
 </div>
 <div className="max-h-[48vh] space-y-3 overflow-y-auto size-8 custom-scrollbar">
 {sessionItems.length === 0 ? (
 <div className="rounded-sm border border-dashed border-border bg-surface p-3 text-sm text-text-muted">
 ...
 className={`w-full rounded-sm border px-3 py-3 text-left transition ${isActive ? "border-primary/30 bg-primary/10" : "border-border bg-surface hover:bg-surface-2"}`}
 >
 <div className="flex items-start justify-between gap-3">
 ...
 <span className="text-[11px] text-text-subtle shrink-0">{formatRelativeTime(session.updatedAt)}</span>
 </div>
 </button>
 );
 })}
 ...
 {loadError ? (
 <div className="mt-4 rounded-sm border border-danger/30 bg-danger/10 h-8 px-3 text-danger">
 ...
 <div className="flex min-h-[50vh] items-center justify-center px-3 text-center">
 <div className="max-w-xl space-y-3">
 <div className="flex size-8 items-center justify-center rounded-sm border border-border bg-surface text-text-main h-8">
 <span className="material-symbols-outlined text-[18px]">chat</span>
 </div>
 <div className="space-y-3">
 <h2 className="text-sm font-semibold text-text-main">Start a conversation</h2>
 ...
 <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 px-3">
 ...
 return (
 <div key={message.id} className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-3`}>
 <div className={`max-w-[min(88%,42rem)] ${isUser ? "rounded-sm bg-surface px-3 py-3.5 text-text-main" : "text-text-main"}`}>
 <div className="mb-1 flex items-center justify-between gap-3">
 <span className="text-xs font-medium">{isUser ? "You" : activeModel?.name || "Assistant"}</span>
 </div>

 {message.attachments?.length ? (
 <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 mt-2">
 {message.attachments.map((attachment) => (
 <a key={attachment.id} href={attachment.dataUrl} target="_blank" rel="noreferrer" className="overflow-hidden rounded-sm border border-border bg-surface-3">
 ...
 <div className="mx-auto mb-3 flex w-full max-w-3xl flex-wrap gap-2 px-3">
 {attachments.map((attachment) => (
 <div key={attachment.id} className="flex items-center gap-2 rounded-sm border border-border bg-surface px-3 h-8">
 ...
 <div className="rounded-sm bg-surface border border-border px-3 pt-3 pb-2">
 <textarea
 value={draft}
 ...
 <div className="flex items-center gap-2">
 <button type="button" onClick={() => fileInputRef.current?.click()} disabled={!activeModel || loadingData} className="size-8 shrink-0 rounded-sm text-text-muted hover:bg-surface-2 hover:text-text-main">
 ...
 <div className="flex items-center gap-2">
 {isSending ? (
 <button type="button" onClick={handleStop} className="size-8 flex shrink-0 items-center justify-center rounded-sm bg-surface-3 text-text-main hover:bg-surface-2">
 <span className="material-symbols-outlined text-[18px]">stop</span>
 </button>
 ) : null}
 <button onClick={sendMessage} disabled={!canSend} className={`h-8 w-8 rounded-sm flex items-center justify-center transition ${canSend ? 'bg-primary text-white hover:bg-primary-hover' : 'bg-surface-3 text-text-muted cursor-not-allowed'}`}>
 ...
 <p className="mx-auto mt-2 max-w-3xl px-3 pb-3 text-center text-[11px] text-text-muted">
