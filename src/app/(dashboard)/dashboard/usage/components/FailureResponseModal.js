    ...
    <Modal
      ...
      title={
        <div className="flex items-center gap-2 min-w-0 pr-4">
          <span className="material-symbols-outlined text-danger text-xl shrink-0">
            error
          </span>
    ...
    >
      {loading ? (
        <div className="flex flex-col items-center justify-center p-12 text-text-muted gap-3">
          <span className="material-symbols-outlined animate-spin text-2xl text-brand-500">
            progress_activity
          </span>
          <span className="text-xs">Fetching failure traces & responses…</span>
        </div>
      ) : !failures.length ? (
        <div className="flex flex-col items-center justify-center p-10 text-text-muted gap-2 border border-dashed border-border rounded-xl">
          ...
          <a
            ...
            className="mt-2 text-xs text-brand-500 hover:underline inline-flex items-center gap-1 font-medium"
          >
            Check Request Details Tab <span className="material-symbols-outlined text-[13px]">open_in_new</span>
          </a>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 max-h-[70vh] min-h-[400px]">
          {/* List of failed instances */}
          <div className="lg:col-span-5 flex flex-col border border-border rounded-xl bg-surface-2/40 overflow-hidden">
            <div className="p-2.5 border-b border-border bg-surface-2/80 flex items-center justify-between">
              ...
              {failures.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full p-3 text-left transition-colors flex flex-col gap-1.5 cursor-pointer",
                      isSelected
                        ? "bg-brand-500/10 border-l-4 border-brand-500 pl-2.5"
          ...
          <div className="lg:col-span-7 flex flex-col border border-border rounded-xl bg-surface-1 overflow-hidden">
            {selected ? (
              <div className="flex flex-col h-full">
                ...
                <div className="p-3 border-b border-border bg-surface-2/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-md border border-border bg-surface-2 hover:bg-surface-3 text-text-main transition-colors cursor-pointer"
                  >
                ...
                {/* Error highlights */}
                <div className="p-3 bg-danger/5 border-b border-danger/20 flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-danger flex items-center gap-1">
                    <span className="material-symbols-outlined text-[14px]">error</span>
                    Error Summary
                  </span>
                  <p className="text-xs font-semibold text-danger break-words">
                ...
                <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-2">
                  ...
                  <pre className="flex-1 max-h-[350px] overflow-auto rounded-lg border border-border bg-surface-2/80 p-3 font-mono text-xs text-text-main whitespace-pre-wrap break-all select-all">
                    {JSON.stringify(
                      selected.response ||
                        selected.providerResponse ||
                        selected.error || {
                ...
                <div className="p-2.5 border-t border-border bg-surface-2/40 flex items-center justify-between">
                  <a
                    href={`/dashboard/usage?tab=details&status=failed&provider=${encodeURIComponent(
                      selected.provider || "",
                    )}&model=${encodeURIComponent(selected.model || "")}`}
                    className="text-xs text-brand-500 hover:underline inline-flex items-center gap-1 font-medium"
                  >
