    ...
    >
      ...
      >
        {imageUrl && !imgError ? (
          ...
          <span className="text-sm font-bold" style={iconCharStyle}>{textIcon}</span>
        )}
      </div>
      ...
      {active && motionOK && (
        <span className="relative flex h-2 w-2 shrink-0">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={pingStyle} />
          <span className="relative inline-flex rounded-full h-2 w-2" style={pingStyle} />
        </span>
      )}
    </div>
  );
