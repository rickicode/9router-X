    ...
    <div className={`group min-w-0 w-full rounded-lg border px-3 py-2.5 sm:w-auto ${borderColor} bg-card/60 transition-colors hover:bg-sidebar/50`}>
      <div className="flex min-w-0 items-center justify-between gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <span className="material-symbols-outlined shrink-0 text-base" style={iconColor ? { color: iconColor } : undefined}>
            {testStatus === "ok" ? "check_circle" : testStatus === "error" ? "cancel" : "smart_toy"}
          </span>
          <div className="flex min-w-0 flex-1 flex-col gap-0.5">
            <code className="w-full truncate text-xs text-text-muted font-mono bg-sidebar px-1.5 py-0.5 rounded select-all" title={fullModel}>{fullModel}</code>
            {model.name && <span className="w-full truncate text-[9px] text-text-muted/70 italic pl-0.5" title={model.name}>{model.name}</span>}
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {onTest && (
            <div className="relative group/btn">
              <button onClick={onTest} disabled={isTesting} aria-label="Test model" className={`p-1 hover:bg-sidebar rounded text-text-muted hover:text-primary transition-opacity ${isTesting ? "opacity-100" : "opacity-100 sm:opacity-0 sm:group-hover:opacity-100"}`}>
          ...
          <div className="relative group/btn">
            <button onClick={() => onCopy(fullModel, `model-${model.id}`)} aria-label="Copy model name" className="p-1 hover:bg-sidebar rounded text-text-muted hover:text-primary">
              <span className="material-symbols-outlined text-base">{copied === `model-${model.id}` ? "check" : "content_copy"}</span>
            </button>
            <span className="pointer-events-none absolute mt-1 top-6 left-1/2 -translate-x-1/2 text-[10px] text-text-muted whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity">
          ...
          {isCustom && (
            <button onClick={onDeleteAlias} className="p-1 hover:bg-red-500/10 rounded text-text-muted hover:text-red-500 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity" title="Remove custom model" aria-label="Remove custom model">
