...
export default function AntigravityToolCard({
 ...
 <span className={`material-symbols-outlined text-[18px] ${ok ? "text-success" : "text-text-muted"}`}>
 ...
 {i < 2 && <span className="material-symbols-outlined text-[18px] text-text-muted">arrow_forward</span>}
 </div>
 );
 })}
 </div>
 ...
 onClick={handleStop}
 disabled={loading}
 className="px-3 h-8 rounded-sm bg-danger/10 border border-danger/30 text-danger font-medium text-sm flex items-center gap-2 hover:bg-danger/10 disabled:opacity-50"
 >
 ...
 className="px-3 h-8 rounded-sm bg-primary/10 border border-primary/30 text-primary font-medium text-sm flex items-center gap-2 hover:bg-primary/10 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 className="w-full min-w-0 px-2 h-8 bg-surface rounded-sm text-xs border border-border focus:outline-none sm:py-2"
 >
 {apiKeys.map((key) => <option key={key.id} value={key.key}>{key.key}</option>)}
 </select>
 ) : (
 <span className="min-w-0 rounded-sm bg-surface/40 px-2 h-8 text-xs text-text-muted sm:py-2">
 ...
 className={`w-full sm:w-auto rounded-sm border px-2 h-8 text-xs sm:py-2 whitespace-nowrap sm:shrink-0 ${hasActiveProviders ? "bg-surface border-border text-text-main hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
 >
 Select
 </button>
 </div>
 ...
 <div className="flex items-center gap-2 px-2 py-2 rounded-sm text-xs bg-warning/10 text-warning border border-warning/30">
 ...
 <span>2. Redirects <code className="text-[11px] bg-surface px-1 rounded-sm">daily-cloudcode-pa.googleapis.com</code> → localhost</span>
