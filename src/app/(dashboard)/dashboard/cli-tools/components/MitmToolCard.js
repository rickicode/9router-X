...
useEffect(() => {
    if (isExpanded) queueMicrotask(() => { loadSavedMappings(); });
  }, [isExpanded]);
 ...
 <span className={`material-symbols-outlined text-text-muted text-[18px] transition-transform ${isExpanded ? "rotate-180" : ""}`}>
 expand_more
 </span>
 </button>
 ...
 {/* Hosts */}
 {mitmHosts.length > 0 && (
 <div className="mt-2 rounded-sm border border-border bg-surface/50 px-2 py-2">
 <p className="text-[11px] font-medium text-text-main/80 mb-1">
 Edit hosts file manually to add the following entries:
 </p>
 <ul className="list-none space-y-3 font-mono text-[11px] text-text-muted break-all">
 {mitmHosts.map((h) => (
 ...
 <span className="text-xs font-medium text-text-main sm:text-right">{model.name}</span>
 ...
 disabled={!dnsActive}
 className={`w-full min-w-0 pl-2 pr-7 h-8 bg-surface rounded-sm border border-border text-xs focus:outline-none sm:py-2 ${!dnsActive ? "opacity-50 cursor-not-allowed" : ""}`}
 />
 ...
 className={`rounded-sm border px-2 h-8 text-xs sm:py-2 ${hasActiveProviders && dnsActive ? "bg-surface border-border hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
 >
 Select
 </button>
 </div>
 ...
 className="flex w-full items-center justify-center gap-1.5 rounded-sm border border-danger/30 bg-danger/10 px-3 h-8 text-xs font-medium text-danger hover:bg-danger/10 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:py-2"
 >
 ...
 onClick={handleDnsToggle}
 disabled={!serverRunning || loading}
 className="flex w-full items-center justify-center gap-1.5 rounded-sm border border-primary/30 bg-primary/10 px-3 h-8 text-xs font-medium text-primary hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:py-2"
 >
 ...
 {warning && (
 <div className="flex items-center gap-2 px-2 py-2 rounded-sm text-xs text-warning">
