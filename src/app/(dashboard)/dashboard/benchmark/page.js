...
export default function BenchmarkPage() {
 ...
 className={`relative flex items-start gap-3 rounded-sm border p-3 text-left cursor-pointer ${
 checked
 ...
 <div
 className="flex items-center justify-between gap-3 p-3 cursor-pointer select-none bg-surface hover:bg-surface-2/60 h-8"
 ...
 className="text-xs text-text-muted hover:text-danger flex items-center gap-1 px-2 py-1 rounded-sm hover:bg-danger/10"
 ...
 {isExpanded ? (
 <div className="p-3 pt-2 border-t border-border bg-surface">
 <div className="flex flex-wrap gap-2">
 {models.map((m) => (
 <span
 key={m.fullId}
 className="inline-flex items-center gap-1.5 rounded-sm border border-primary/30 bg-surface px-2.5 py-1 text-xs text-text-main font-medium group"
 >
 ...
 className="text-text-muted hover:text-danger size-8 rounded-sm hover:bg-surface-2"
 ...
 <tr>
 <th className="h-8 px-3 text-xs font-medium text-text-muted">Akun</th>
 <th
 className="h-8 px-3 cursor-pointer hover:text-text-main text-xs font-medium text-text-muted"
 ...
 <th className="h-8 px-2 text-xs font-medium text-text-muted">Suite</th>
 <th
 className="h-8 px-2 cursor-pointer hover:text-text-main text-xs font-medium text-text-muted"
 ...
 <th className="h-8 px-3 max-w-[220px] text-xs font-medium text-text-muted">Pesan Respon / Error</th>
 <th
 className="h-8 px-2 text-right cursor-pointer hover:text-text-main text-xs font-medium text-text-muted"
 ...
 <th className="h-8 px-2 text-center text-xs font-medium text-text-muted">Format</th>
 </tr>
 </thead>
 ...
 >
 <td className="h-8 px-3 font-mono text-[11px] max-w-[100px] truncate text-sm" title={row.account_name}>
 {row.account_name || "-"}
 </td>
 <td className="h-8 px-3 font-medium max-w-[140px] truncate text-sm" title={row.model}>
 {row.model}
 </td>
 <td className="h-8 font-medium text-[11px] text-text-muted px-3 text-sm">
 {row.suite}
 </td>
 ...
 <td
 className={`h-8 px-3 max-w-[240px] truncate font-mono text-[11px] ${
 ...
 <td className="h-8 px-3 text-right font-medium text-sm">
 ...
 <td className="h-8 px-3 text-right font-mono text-text-muted text-sm">
 {row.ttft_ms ? `${row.ttft_ms}ms` : "-"}
 </td>
 <td className="h-8 px-3 text-right font-mono text-text-muted text-sm">
 {row.total_ms ? `${row.total_ms}ms` : "-"}
 </td>
 <td className="h-8 px-3 text-right font-mono font-medium text-sm">
 {row.tps ? `${row.tps}` : "-"}
 </td>
 <td className="h-8 px-3 text-center font-mono text-[11px] text-text-muted text-sm">
 {row.format || "-"}
 </td>
 </tr>
 );
 ...
 <tr>
 <td colSpan="10" className="py-3 text-center text-text-muted h-8 px-3 text-sm">
 ...
 <th className="h-8 px-3 text-xs font-medium text-text-muted">PONG Gate (Passed/Total)</th>
 <th className="h-8 px-3 text-right text-xs font-medium text-text-muted">Median Kualitas</th>
 <th className="h-8 px-3 text-right text-xs font-medium text-text-muted">Median TTFT (Byte Pertama)</th>
 <th className="h-8 px-3 text-right text-xs font-medium text-text-muted">Median Total Latensi</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border-subtle text-xs">
 {daily.map((row) => (
 <tr key={`${row.provider}-${row.model}`} className="hover:bg-surface-2">
 <td className="h-8 px-3 font-medium text-text-main text-sm">{row.model}</td>
 <td className="h-8 px-3 text-sm">
 {row.pong_total ? (
 ...
 {daily.length === 0 ? (
 <tr>
 <td colSpan="5" className="py-3 text-center text-text-muted h-8 px-3 text-sm">
 ...
 {isCurrent ? (
 <span className="px-1.5 py-1 rounded-sm bg-primary/10 text-primary font-medium text-[11px]">
 SEDANG DILIHAT
 </span>
 ) : null}
 </div>
 ...
 className="size-8 shrink-0 rounded-sm text-text-muted hover:bg-surface-2 hover:text-danger"
 ...
 <div className="px-3 border-b border-border flex items-center justify-between gap-3 bg-surface h-8">
 ...
 <div className="flex items-center justify-between pb-1.5 border-b border-border text-[11px] text-text-muted h-8">
 ...
 className={`flex items-center justify-between gap-2 p-3 rounded-sm text-xs cursor-pointer border ${
 isModelChecked
 ...
 Suite: <span className="font-medium">{inspectAttempt.suite}</span> (Rep {inspectAttempt.rep || 1}) · Akun:{" "}
 ...
 {/* Modal Body */}
 <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 text-xs font-mono bg-surface">
 {/* Telemetry Chips */}
 ...
 <pre className="rounded-sm bg-danger/10 border border-danger/30 p-3 text-danger whitespace-pre-wrap break-all text-[11px] border-border bg-surface font-mono text-xs text-text-main">
 {inspectAttempt.error}
 </pre>
 </div>
 ) : null}
 ...
 className={`flex items-center justify-between p-3 rounded-sm border cursor-pointer bg-surface ${
 ...
 {isJobRunning ? (
 <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-sm bg-success/10 border border-success/30 text-success text-[11px] font-medium">
 ...
 <div className="px-3 py-3 border-b border-border bg-surface flex flex-wrap items-center gap-2">
 ...
 {/* Log tail body */}
 <div ref={logScrollRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-surface font-mono text-[11px]">
 {logEntries.map((row) => {
 ...
 className="w-full flex items-center gap-2 px-3 h-8 text-left hover:bg-surface-2/60"
 >
 <span className="text-text-muted text-xs w-16 shrink-0">
 {row.created_at ? new Date(row.created_at).toLocaleTimeString() : "-"}
 </span>
 <span className={`px-1.5 py-1 rounded-sm border text-[11px] font-medium shrink-0 ${statusColor}`}>
 ...
 {expanded ? (
 <div className="border-t border-border px-3 h-8 space-y-3">
 <div>
 ...
 {logEntries.length === 0 ? (
 <div className="py-3 text-center text-text-muted font-sans text-xs">
