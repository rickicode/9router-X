...
cp .env.example .env
npm install
PORT=10128 NEXT_PUBLIC_BASE_URL=http://localhost:10128 npm run dev   # dev (webpack, port 20127 by default via next dev)
npm run build && PORT=10128 HOSTNAME=0.0.0.0 npm run start           # production
```
- Bun variants: `npm run dev:bun` / `build:bun` / `start:bun`.
- Default runtime port is **10128** (dashboard at `/dashboard`, API at `/v1`).
