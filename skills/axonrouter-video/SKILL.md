---
name: axonrouter-video
description: Generate videos via AxonRouter /v1/videos/generations using xAI Grok Imagine (grok-imagine-video). Async job flow - submit, poll request_id until done, download MP4. Use when the user wants to create, generate, or render a video, text-to-video (txt2vid), or image-to-video.
---

# AxonRouter — Video Generation (xAI Grok Imagine)

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/axonrouter/refs/heads/master/skills/axonrouter/SKILL.md for setup.

Requires a connected **xAI account** in the AxonRouter dashboard — either **Grok Build OAuth** (SuperGrok / X Premium+ subscription sign-in) or a direct **xAI API key** from console.x.ai. The two are separate auth types with separate billing; the dashboard shows which one each connection uses.
...
# → {"request_id":"abc123"}   (response header x-axonrouter-connection-id: <id>)
```
...
- Jobs are **account-bound** upstream: poll with the same connection that created the job (`x-connection-id` header, value from the create response's `x-axonrouter-connection-id`).
